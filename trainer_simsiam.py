...
#                 print(losses_night)
            ...
            if self.opt.only_depth_encoder:
                ...
                if self.opt.use_init_pred:
                    loss = losses + self.lamda1*losses_day["loss"] + self.lamda1*losses_night["loss"] + self.lamda2*losses_day_init["loss"] + self.lamda2*losses_night_init["loss"]
                else:
            ...
            if batch_idx % 10 == 0 and not self.opt.only_depth_encoder:
                if self.opt.use_init_pred:
                    self.log_time(batch_idx, duration, self.lamda1*losses_day["loss"].cpu().data+self.lamda1*losses_night["loss"].cpu().data+losses.cpu().data+self.lamda2*losses_day_init["loss"].cpu().data+self.lamda2*losses_night_init["loss"].cpu().data)  
                else:
                ...
                if "depth_gt" in inputs:
                    ...
                    if self.opt.use_init_pred:
                        self.compute_depth_losses(inputs, outputs_init, losses_day_init)
                        self.compute_depth_losses(inputs, outputs_night_init, losses_night_init)
